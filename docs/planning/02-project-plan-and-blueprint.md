...
| R-03 | Model adds no real edge over intuition | High | **Confirmed, partially** *(2026-08-11)* | Backtest before trusting (B7) — it ran, and found the edge is real on average (beats B0) but absent exactly where decisions are made (loses to a trailing-form heuristic at the head of the ranking). See **R-15a** and [DL-21](00-decision-log.md#dl-21--the-v1-forecast-beats-price-and-loses-to-recent-form-reported-not-tuned). Scout UI retains standalone value regardless | Owner |
...
| R-15 | ~~**Expected points collapses onto price**~~ **Measured, 2026-08-11 — did not materialise as stated.** R² of xP on (price, position) = 0.483 ([model-card.md](../../data/gold/season=2026-27/model-card.md), verdict "informative") — the model adds real information beyond price. **A related but distinct failure materialised instead**, captured as **R-15a** below | Was High; the literal risk is now closed by measurement | — | Diagnostic ran as designed in E0-S5 and again in E3's backtest; the R-15 mechanism (price as a signal producing a flat objective) did not occur | Owner |
| R-15a | **New, 2026-08-11 — the model under-discriminates at the head of the ranking specifically** — MAE is best-in-class (1.936, beats B0's 1.965) but top-20 precision is worst-in-class (0.00 vs B0's and the model-free benchmark's 0.05). Shrinking every thin estimate toward a position prior avoids being badly wrong on average and avoids distinguishing anyone at the top, which is backwards for how the tool is used (nobody acts on the whole ranking). See [DL-21](00-decision-log.md#dl-21--the-v1-forecast-beats-price-and-loses-to-recent-form-reported-not-tuned) | High — the optimiser and captaincy calls act on exactly this part of the ranking | Confirmed, not hypothetical | Debt **D-13** ([E0 §6](epics/E0-steel-thread-gw1.md#6-technical-debt-register)); E4 must not justify a hit, chip or wildcard on `xp_v1` alone until top-20 precision beats B0 (gate written into [E4 §0](epics/E4-decision-engine.md#0-gate-carried-in-from-e3--read-before-starting)); next model work targets head discrimination, not MAE | Owner |
