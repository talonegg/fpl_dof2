...
| D-07 | ~~No entity resolution~~ **Mechanism closed by E5-S1** — three-tier resolution (deterministic/fuzzy/override), a hard failure on cross-source ID conflict, and an unmatched-rate quality gate all exist and are tested. **Still not fully closed**, for a better-understood reason than before: the historical backfill it needs to resolve against now exists and is tested ([DL-29](../00-decision-log.md#dl-29)), but nothing downstream reads the resolved metrics, so the resolution is still never exercised against real historical data end to end | The invisible-mismatch risk this debt named is mitigated for live runs; it remains untested against real historical data | **D-22** — closes when the feature store consumes `player_metric` and the backtest re-runs |
...
| D-22 | **Nothing consumes the conformed advanced metrics.** `player_metric` and `player_advanced` are written to silver by the transform stage and read by **no module in the project**; the feature store builds every feature from `player_gameweek` alone. Separately, both scraped sources conform *running season totals* at `scope="season"`, which are legitimate only as a **prior-season** feature — a finished season's total is unusable at any deadline inside that season (Invariant 5) | This, not the backfill, is why E5's "backtest metrics measurably improve" cannot be evaluated. The chain from a backfilled xG figure to a backtest number is broken one link downstream of where D-20 said it was, and no amount of crawling repairs it | A feature-store story: decide the prior-season feature design, add the `player_metric` join to `forecast/features.py` with an explicit `knowable_at`, then re-run the backtest against the DL-21 baseline. E5's DoD and D-07 both close or fail on the result |
