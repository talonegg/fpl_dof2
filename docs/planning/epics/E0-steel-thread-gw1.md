...
| D-08 | ~~No automation; every run is manual~~ **Closed by E7.** Seven workflows — `ci`, `ingest-fast`, `ingest-slow`, `pipeline`, `deploy`, `backtest`, plus a reusable `alert-on-failure` — cover ingestion, decision, publication and alerting on a deadline-aware schedule. See [DL-42](../00-decision-log.md#dl-42) | Not viable across 38 deadlines, especially given AEST timings. **What is not yet verified is the live behaviour** — real cron firing, real Pages deploy, real `GITHUB_TOKEN` push — only local/simulated so far (DL-42's Consequences) | **E7** — closed in code; the DoD's "one full week, zero manual intervention" is the live-verification step still ahead |
| D-09 | Uncertainty is a heuristic band, not a modelled variance | The eventual risk dial needs real variance | E3 |
| D-10 | ~~**No CI.**~~ **Closed by E7.** `ci.yml` runs on push and pull request, invoking the exact `fpl-dof` entry points a developer does — lint, mypy, the full pytest suite, the 100% rules-coverage gate, and the web typecheck/build/test chain. No CI-only branch exists anywhere in the pipeline source (NFR-09). See [DL-42](../00-decision-log.md#dl-42) | Nothing proved the code ran anywhere but one machine. Closed on 2026-08-16, before the dated carve-out in [charter §13](../01-project-charter.md#the-one-dated-exception--e0) expires on 22 Aug — the carve-out is retired unused rather than run out | **E7** — closed, the E0 code path runs in CI unchanged |
...
| D-14 | ~~**Minutes-model calibration is unmeasured**~~ **Closed by E10-S1.** The harness now scores M1's `{0, 1-59, 60+}` distribution against a reconstructed E0 status-flag haircut, per position and per observed band, over the whole population rather than the scored subset. **M1 beats the haircut in every position** (Brier 0.359 vs 0.445, skill +0.193), so E3-S3's acceptance criterion is met by a number rather than by a ticked box. See [DL-48](../00-decision-log.md#dl-48) | Measured is not good: the by-band split shows M1 beats the haircut by being far better about non-appearances and **worse** about the players who lasted the hour (0.421 vs 0.298) — the head of the ranking is made of exactly those players, and that deficit is now E10-S2's target | **Closed** by `tests/test_minutes_calibration.py`; the `long`-band deficit carries forward into E10-S2 |
...
| D-25 | ~~**The improved model is not the one that ships.**~~ **Closed by [DL-46](../00-decision-log.md#dl-46) (E9-S1).** ([DL-33](../00-decision-log.md#dl-33), [DL-34](../00-decision-log.md#dl-34)). Narrowed twice, then closed. The xG *signal* turned out to be available from the official feed itself, so expected goals were wired into `xp_v1`'s goal-involvement model, backtested (Spearman 0.2255 → 0.2307, MAE-skill 0.0099 → 0.0196, calibration 0.60 → 0.70) and **promoted** in the shipped config (DL-34). What remained was the live-promotion seam DL-33 named — larger than that entry assumed, because `xp_v1.to_frame` emitted only `xp_next` and a variance while both MILPs require `xp_horizon` and a per-gameweek μ[p,w]. E9-S1 built that scorer (`forecast/horizon.py`, the single copy the chip replay now also uses) and switched `stages/forecast.py` to `xp_v1` as the default publish target | The measured xG improvement now reaches what `run` publishes. DL-46 records why this is treated as a **bug fix** rather than a DP-08 model promotion: `xp_v1` was already selected and graded, and the defect was that a *different, less-capable* model shipped. Distinct from D-23 (which blocks the *scraped* provenance) and from D-13, whose top-20 bar is unchanged — [DL-21](../00-decision-log.md#dl-21)'s guardrail (no −8 hit, chip or wildcard on `xp_v1` alone until top-20 precision beats B0) stands and is restated on every model card | **Closed.** `xp_v0` is retained as the documented cold-start fallback only, fired below `forecast.published.cold_start_minimum_gameweeks` and named in the model card, the stage metrics and the artefact's own `model` column (DP-15). A parity test holds the live horizon scorer to the backtest's single-gameweek `xp_v1` numbers under league-average opposition (`tests/test_xp_v1_live.py`). The harness's own fixture blindness is E9-S2, not this item |
| D-26 | ~~**No backtest this project has ever run resolved a single fixture.**~~ **Closed by [DL-52](../00-decision-log.md#dl-52).** The archive adapter wrote `"team_id": None` on every `player_gameweek` row, so `fixture_calendar` returned an empty calendar, `attach_fixtures` took its stated-absence branch, and **100% of scored observations were predicted against league-average opposition** — which `backtest.json` warned in every run and nobody read. Found by [E10-S4](E10-discrimination-at-the-head.md) ([DL-51](../00-decision-log.md#dl-51)). The club was never missing from the archive: `merged_gw.csv` carries the club *name* on every row and `teams.csv` carries the season's club list. It is resolved through that list to FPL's stable club `code`, not the season-local id — **eight to ten of the twenty ids point at a different club each year**, so an id would have handed one club's form to another. `opponent_team_id` is relabelled with it, because two sides of a fixture in two id spaces join a club to a stranger | **E9-S2's acceptance is now met**: fixture coverage is **1.0**, all 114 gameweeks build a calendar, and M2 fits 2,280 real matches (3 seasons x 20 clubs x 38) where it previously fitted **none**. Every figure in [DL-48](../00-decision-log.md#dl-48), [DL-49](../00-decision-log.md#dl-49), [DL-50](../00-decision-log.md#dl-50) and DL-51 was measured under league-average opposition and is re-based, not merely improved: overall Spearman 0.231 -> 0.250 and DEF 0.163 -> 0.219 with **no model change at all**, while top-20 precision *falls* 0.127 -> 0.122 | **Closed.** Regression guards in `tests/test_archive_source.py` assert every archive row names its club, that the id is the stable code and is the same integer for a club renumbered between two seasons, that club and opponent share one id space, and that the harness's calendar is non-empty |

| D-27 | ~~**The XI floor drifted onto a different quantity.**~~ **Closed by [DL-66](../00-decision-log.md#dl-66).** E9-S1 redefined `start_probability` as M1's P(60+ minutes) while `minimum_start_probability_for_xi` kept its value and its P(start) meaning; four gameweeks in, every forward sat below the floor and both CI runs on 2026-09-16 aborted at `optimise` | No transfer recommendation was published, and the from-scratch solve that failed was not even the in-season decision (see D-28) | **Closed.** `start_probability` is P(appears at all) in every model, pinned by `tests/test_xp_v1_live.py` |
| D-28 | ~~**Advice was never persisted, so E1-S5's reconciliation never ran in CI.**~~ **Closed by [DL-69](../00-decision-log.md#dl-69).** The previous `week.json` was the only record of what was advised; CI starts every run from a fresh checkout, so it never existed there, and the ingest workflows never received the team ID at all, so no entry snapshot reached bronze | E8 §3's decision log — the dataset that decides whether the model or the owner is better — was empty for GW1 to GW4 | **Closed.** The `ledger` branch keeps advice per gameweek, `log.json` publishes played/scored/advised, and both ingest workflows pass the repository variable |
