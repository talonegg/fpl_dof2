...
| D-07 | ~~No entity resolution~~ **Mechanism closed by E5-S1** — three-tier resolution (deterministic/fuzzy/override), a hard failure on cross-source ID conflict, and an unmatched-rate quality gate all exist and are tested. **Not fully closed**: see D-20 — the mechanism has nothing to resolve against in the backtest yet, since none of the three sources backfills history | The invisible-mismatch risk this debt named is mitigated for live runs; it remains untested against real historical data | **E5** — mechanism done; D-20 tracks the remainder |
...
| D-18 | **The simulation re-rank is not exercised by the backtest.** E4-S4a's acceptance criterion is that the re-rank changes at least one chip recommendation *in the backtest*. It is demonstrated instead by a constructed unit test in which the Bench Boost timings tie exactly on expected points and the dial's percentile separates them | The re-rank is proven not to be inert, and is **not** proven to be an improvement on real gameweeks. Nothing measures whether it helps | A backtest-harness change: run `optimise.plan.build_plan` inside the walk-forward loop and score chip timing against what actually happened. Realistically an E6/E7 job, since it needs real gameweeks to have resolved |
...
| D-20 | **None of E5's three new sources backfills history.** Understat, FBref and the odds adapter all fetch current-season data only; `enabled_by_default = False` and no season-backfill path exists for any of them. The walk-forward backtest (2022/23–2025/26) therefore has zero rows from any of the three | E5's own stated purpose ("its true test is whether backtest metrics improve") cannot be evaluated — not "tested and found not to help", but genuinely untested. D-07's entity resolution has nothing real to resolve against yet either | A follow-up story, not automatically E6/E7: needs a backfill path per adapter (Understat and FBref both publish historical seasons; the odds provider likely does not, which would leave M2's improvement untestable regardless) before E5's DoD can be honestly ticked |
