...
| D-25 | **The improved model is not the one that ships.** ([DL-33](../00-decision-log.md#dl-33), [DL-34](../00-decision-log.md#dl-34)). Narrowed twice. The xG *signal* turned out to be available from the official feed itself, so expected goals were wired into `xp_v1`'s goal-involvement model, backtested (Spearman 0.2255 → 0.2307, MAE-skill 0.0099 → 0.0196, calibration 0.60 → 0.70) and **promoted** in the shipped config (DL-34). What remains is the live-promotion seam DL-33 named — but it is **larger than that entry assumed**: `stages/forecast.py` still runs `xp_v0`, and swapping in `xp_v1` cannot be a one-line read because `xp_v1.to_frame` emits only `xp_next` and a variance, while both the squad MILP and the weekly plan MILP require `xp_horizon` and a per-gameweek `gw_n` column per fixture. `xp_v1` has no fixture-aware horizon scorer | So the measured xG improvement lives in the backtest, not in what `run` publishes. The conceptual design §5 now consumes xG in the component model but the live artefact does not. Distinct from D-23 (which blocks the *scraped* provenance and its extra fields — npxG, shot detail — not xG itself) and D-13 (the top-20 bar xG does not clear) | Build a fixture-aware horizon scorer that wraps the component models to emit `xp_horizon` and per-gameweek μ[p,w] with variance, then switch `stages/forecast.py` to it for the in-season path with `xp_v0` retained for the preseason cold start. Deliberately **not** rushed against the GW1 deadline (DL-10): a contract-breaking swap would be worse than an honest cold-start forecast that carries the D-13 caveat |
