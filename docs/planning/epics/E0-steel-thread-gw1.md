...
| D-25 | **The live forecast consumes no external data; §5's xG-based M2/M3/M4 are implemented only in the backtest-only `xp_v1`** ([DL-33](../00-decision-log.md#dl-33)). `stages/forecast.py` runs `xp_v0.build_forecast`, whose `ForecastInputs` has no `player_metric` field and never reads `Table.PLAYER_METRIC`; the D-22 prior-season consumer lives in `xp_v1.ComponentPredictor`, which only `stages/backtest.py` instantiates. So Understat/FBref reach neither the initial squad MILP nor the weekly plan MILP in `run` — both read `expected_points.parquet`, which `xp_v0` writes from FPL data alone | The conceptual design §5 describes a model that does not ship. Because both optimisers consume the forecast and nothing source-specific (correct, DP-02/Invariant 1), the *entire* fix is one seam — make the live forecast consume `player_metric` — after which both models inherit it through the contract they already read. Distinct from D-23 (no data to consume) and D-13 (no evidence to promote on) | Blocked behind **D-23** and **D-13**, in that order. When both clear: add `metrics` to `ForecastInputs`, read `Table.PLAYER_METRIC` in `stages/forecast.py` (`read_table_optional`, degrade per DP-15), and promote `xp_v1` to the in-season live path rather than porting its join into `xp_v0` — see [DL-33](../00-decision-log.md#dl-33) for why porting is rejected |
